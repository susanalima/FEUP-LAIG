...
class MySceneGraph {
    ...
    parseXMLFile(rootElement) {
        ...
        // <scene>
        var index;
        if ((index = nodeNames.indexOf("scene")) == -1) //tag index not found
    ...
    parseIllumination(illuminationNode) {
      ...
      parseViews(viewsNode) {
        ...
        for (var i = 0; i < children.length; i++) {
            ...
            if (viewId == null)
                return "no ID defined for perspective/ortho";

            // Checks for repeated IDs.
            if (this.views[viewID] != null)
                return "ID must be unique for each perspective/ortho (conflict: ID = " + viewID + ")";

            var near = this.reader.getFloat(children[i], 'near');
            if (near == null)
                return "no near defined for perspective/ortho";

            var far = this.reader.getFloat(children[i], 'far');
            if (far == null)
                return "no far defined for perspective/ortho";
            ...
            if(children[i].nodeName == "perspective")
            {
                ...
                if(grandChildren[0].nodeNames != "from" && grandChildren[1].nodeNames != "to")
                    return "children of perspective have incorrect tags";
